...
for(ilist in 1:length(all_figs)){
	met=all_figs[[ilist]]
	result=rbind(result, c(met$figb[1,"R5"], met$figb[2,"R5"], met$gamma, match(met$dirname, dirnames)))
}
...
# centers=barplot(result2,main="", ylab="Emissions (Gg)", beside=TRUE,col=rep(c("#999999", "#E69F00", "#56B4E9", "#009E73"),7),bty="n",ylim=c(0,1400),border=1,xlim=c(1.0,20))
# box()
...
mtext("Emissions in Region R5 using different priors and γ",side=3,line=0)

screen(2)
par(mai=c(0.1, 0.5, 0.15, 0), mgp=c(1.2, 0.2, 0), tcl=-0.2, ps=10)
centers=barplot(result2,main="", ylab="Emissions (Gg)", beside=TRUE,col=rep(c("#999999", "#E69F00", "#56B4E9", "#009E73"),7),bty="n",ylim=c(0,1400),border=1,xlim=c(1.0,15))
box()
abline(v=c(5.5,10.5) ,lty=2)

#text(x=3,y=1300,"Scarpelli (2020)")
# text(x=3,y=1300,"R6 x 0.13")
# text(x=8,y=1300,"R6 x 0.13")
text(x=8,y=1300,"Mexico x 1.5")
# text(x=13,y=1300,"R6 x 0.13")
text(x=13,y=1300,"R5 x 2.0")
# text(x=22.5,y=750,"")
...
map("world",add=T)
plot.box(c(-94.5,-91.9,17.3,18.6),lwd=2)