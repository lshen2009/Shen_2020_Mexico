...
mtext(bquote("Gas production, flaring radiant heat and "*NO[2]*" in easterm Mexico"),side=3,line=0)

screen(2)
par(mai=c(0.1, 0.1, 0.2, 0.1), mgp=c(1.3, 0.2, 0), tcl=-0.2, ps=9)
ss=load("~/Documents/CH4_Mexico_archive/Data/Well_production/CNH_historical/grid_gas_production.Rdata")#unit is 10^9 m3 per month
...
text(x=0.1,y=-0.08,bquote("[ "*10^9~m^3*month^-1*" ]" )  )
...
map("world",add=T)
box(); fun()
title(bquote("(c) TROPOMI "*NO[2]*""), font.main=1, cex.main=1,line=0.3)
